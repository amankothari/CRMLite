starter.controller('AccountCotroller', function ($scope, $ionicSlideBoxDelegate, $ionicModal, $ionicHistory, notification, $location, $timeout, getsetService, localStorageService, $window,XrmServiceToolkit_Soap, $ionicPopup, $rootScope ) {
    ...
    function CrmOneEntity(response, bool) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < a.length; j++) {
                ...
                //if (a[j].childNodes[0].innerText == "address1_line1") { newobj.address1_line1 = a[j].childNodes[1].innerHTML }
                ...
                //if (a[j].childNodes[0].innerText == "address1_country") { newobj.address1_country = a[j].childNodes[1].innerHTML }
    ...
    $scope.Details = function (data) {
        ...
        else if (getsetService.Getdata().redirecturlphone) {
            var json = { "fromid": data.accountid, "from": data.Name };
            getsetService.Setdata(json);
    ...
    function AddAccount() {
        ...
        if ($scope.accountData.revenue != undefined) { createContact.attributes["revenue"] = { value: $scope.accountData.revenue, type: "Money" }; }   
        }                    
    }
    /// <summary>      
    /// <returns>SOAP response.</returns>
