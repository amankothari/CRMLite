starter.controller('OpportunityController', function (getsetDashboard, $scope, $ionicSlideBoxDelegate, $ionicModal, $ionicHistory, TimesheetService, notification, $location, $timeout, getsetService, authService, localStorageService, $window, $rootScope, $ionicPopup, XrmServiceToolkit_Soap, $stateParams) {
    $scope.showoppo = "Add Opportunity";
    ...
    function RequestOppo(bool) {
        ...
        body.push('</a:Values>');
        body.push('</a:ConditionExpression>');
        if (getsetDashboard.Getdata().SetWon != undefined) {
        body.push('<a:ConditionExpression>');
        body.push('<a:AttributeName>statecode</a:AttributeName>');
        body.push('<a:Operator>Equal</a:Operator>');
        body.push('<a:Values xmlns:c="http://schemas.microsoft.com/2003/10/Serialization/Arrays">');
        body.push('<c:anyType i:type="d:int" xmlns:d="http://www.w3.org/2001/XMLSchema">' + getsetDashboard.Getdata().SetWon + '</c:anyType>');
        body.push('</a:Values>');
        body.push('</a:ConditionExpression>');
    ...
    $scope.Details = function (data) {
        if ($stateParams.type) {
            if ($stateParams.type.split('{').length == 2 && $stateParams.type.split('{')[1] == 'regarding' && $stateParams.type.split('{')[0] == "4212") {
    ...
    $scope.$on('$ionicView.enter', function () {
        ...
        if (getsetService.Getdata().opportunityid != undefined) { $scope.oppoData = getsetService.Getdata() }
        else { $scope.oppoData = getsetService.Getdata(3); }
    ...
    function AddOpportinuty() {
        ...
        function createupdate() {
            ...
            $scope.oppoData.parentaccountid? createContact.attributes["parentaccountid"] = { id: $scope.oppoData.Account.parentaccountid, logicalName: "account", type: "EntityReference" }:""
            $scope.oppoData.parentcontactid? createContact.attributes["parentcontactid"] = { id: $scope.oppoData.Contact.parentcontactid, logicalName: "contact", type: "EntityReference" }:""
            ...
            $scope.oppoData.statuscode ? createContact.attributes["statuscode"] = { value: parseInt($scope.oppoData.statuscode ? $scope.oppoData.statuscode : 1), type: "OptionSetValue" } : ""
    ...
    $scope.Edit = function (editData) {
        $scope.showoppo = "Edit Opportunity"; 
        try {
            $rootScope.show('Please wait...');
            ...
            $scope.format = {
                ...
                "Account": { "parentaccountid": retrievedContact.attributes.parentaccountid == undefined ? undefined : retrievedContact.attributes.parentaccountid.id, "parentaccountname": retrievedContact.attributes.parentaccountid == undefined ? undefined : retrievedContact.attributes.parentaccountid.name },
                "Contact": { "parentcontactid": retrievedContact.attributes.parentcontactid == undefined ? undefined : retrievedContact.attributes.parentcontactid.id, "parentcontactname": retrievedContact.attributes.parentcontactid == undefined ? undefined : retrievedContact.attributes.parentcontactid.name },
